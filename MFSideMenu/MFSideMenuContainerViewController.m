...
- (UIPanGestureRecognizer *)leftMenuPanGestureRecognizer {
  UIPanGestureRecognizer *recognizer = [[UIPanGestureRecognizer alloc]
                                        initWithTarget:self action:@selector(handleLeftMenuPan:)];
...
- (UIPanGestureRecognizer *)rightMenuPanGestureRecognizer {
  UIPanGestureRecognizer *recognizer = [[UIPanGestureRecognizer alloc]
                                        initWithTarget:self action:@selector(handleRightMenuPan:)];
  [recognizer setMaximumNumberOfTouches:1];
...
- (void)addGestureRecognizers {
  [self addCenterGestureRecognizers];
  [menuContainerView addGestureRecognizer:[self leftMenuPanGestureRecognizer]];
  [menuContainerView addGestureRecognizer:[self rightMenuPanGestureRecognizer]];
}
...
- (void)removeCenterGestureRecognizers
{
  if (self.centerViewController)
  {
    ...
    [[self.centerViewController view] removeGestureRecognizer:[self leftMenuPanGestureRecognizer]];
    [[self.centerViewController view] removeGestureRecognizer:[self rightMenuPanGestureRecognizer]];
  }
}
...
- (void) handleLeftMenuPan:(UIPanGestureRecognizer *)recognizer {
  ...
  if(self.panDirection == MFSideMenuPanDirectionRight) {
    [self handleRightPan:recognizer];
  } else if (self.panDirection == MFSideMenuPanDirectionLeft && self.menuState == MFSideMenuStateLeftMenuOpen) {
    [self handleLeftPan:recognizer];
  }
}

- (void) handleRightMenuPan:(UIPanGestureRecognizer *)recognizer {
  UIView *view = [self.centerViewController view];
